...
function AppContent() {
  ...
  // Добавлем проверку обновлений
  useEffect(() => {
    const checkForUpdates = async () => {
      try {
        const response = await fetch('https://api.github.com/repos/Kyurenoxd/osl/releases/latest')
        const data = await response.json()
        const latestVersion = data.tag_name.replace('v', '') // Убираем 'v' из версии если она есть
        const currentVersion = await window.electron.getVersion()
        
        if (latestVersion !== currentVersion) {
          const shouldUpdate = window.confirm('New version available! Would you like to download it?')
          if (shouldUpdate) {
            window.electron.openExternal('https://github.com/Kyurenoxd/osl/releases/latest')
          }
        }
      } catch (error) {
